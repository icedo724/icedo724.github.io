...
## 사내 거래처 데이터 베이스 구축 및 분석
* 사내 의료 영업 데이터와 HIRA(건강보험심사평가원) 공공 데이터를 결합하여 데이터 분석 환경 구축.
...
* 챔피언 픽률, 아이템 트리, 승률 간의 상관관계를 통계적으로 분석하여 게임 내 최적의 메타(Meta) 인사이트 도출.
...
* 주말 이벤트('썬데이 메이플')가 유저 성장 속도에 미치는 영향을 정량적인 수치로 검증.
...
## Kaggle Competitions
* 캐글 플랫폼을 활용한 머신러닝/데이터 분석 대회 참여 및 코드 기록.
